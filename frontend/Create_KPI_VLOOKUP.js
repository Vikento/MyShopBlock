...
import * as myConstClass from "./myConstClass.js";
 ...
 function Create_KPI_VLOOKUP({base}){
    ...
    // check if the table NAME_TABLE_KPI_VLOOKUP exist
    const table_Lookup = base.getTableByNameIfExists(myConstClass.NAME_TABLE_KPI_VLOOKUP);
    
    // const to manage the dialogue during the creation of the new KPI:
    ...
    if (session.currentUser) {
       ...
       // the Vlookup KPI and we hidde the buttom
        if (createRecordsCheckResult.hasPermission){
           ...
           // if it doesn't exist we just create a button for the creation of the table  NAME_TABLE_KPI_VLOOKUP
            if (!table_Lookup){

                return (               
                    <React.Fragment>
                    <Button onClick={() => setIsDialogOpen(true)}>Create VLookup Table </Button>
                    {isDialogOpen && (
                        <Dialog onClose={() => setIsDialogOpen(false)} width="600px" position="fixed" bottom="-100" right="0">
                        <Dialog.CloseButton />

                        <Heading>Create VLookup Table</Heading>
                        <Text variant="paragraph">
                        Please confirm you want to create the cable for VLookup Table
                        </Text>
                
                        <p> </p>
                        <div >
                            <Button onClick={() => setIsDialogOpen(false)} text-align= "center"  margin=" 0 auto" >Cancel</Button>
                            <Button onClick={() => CreateNewTable()}  text-align= "center"  margin=" 0 auto">Create Table !</Button>
                        </div>
                        </Dialog>
                    )}
                </React.Fragment>
                ...
                for  (let i = 0; i < base.tables.length; i++) {
                    ...
                    // we take only the table with at least 2 numeric field
                    let nb_numeric_field_exist = 0;

                    for (let t = 0; t < base.tables[i].fields.length; t++) {
                        if ((base.tables[i].fields[t].type == FieldType.COUNT) || (base.tables[i].fields[t].type== FieldType.CURRENCY) || (base.tables[i].fields[t].type == FieldType.DATE)||
                        (base.tables[i].fields[t].type == FieldType.DATE_TIME)||(base.tables[i].fields[t].type == FieldType.DURATION)||(base.tables[i].fields[t].type== FieldType.FORMULA)||
                        (base.tables[i].fields[t].type == FieldType.NUMBER)||(base.tables[i].fields[t].type== FieldType.PERCENT)||(base.tables[i].fields[t].type == FieldType.RATING)){
                            nb_numeric_field_exist = nb_numeric_field_exist + 1 ; }
                      }

                    if (nb_numeric_field_exist > 1) {
                            my_table_list.push({
                ...
                const [my_table_list_value, setmy_table_list_value] = useState(my_table_list[0].label);
                ...
                for  (let i = 0; i < base.tables.length; i++) { 
                    if (base.tables[i].name == my_table_list_value){
                        for (let j = 0; j < base.tables[i].fields.length; j++) { 
                            my_field_list.push({
                                value : base.tables[i].fields[j].id, 
                ...
                //-----------------------fill the 2nd select button for the field name base on the previous table name-------------------
                // ------------ same list of fields from my my_field_list, but limited with COUNT, CURRENCY , DATE , DATE_TIME , DURATION , FORMULA 
                ...
                const [valuemy_field_list, setmy_field_list] = useState(my_field_list_numeric[0].value);
                ...
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -- - - - -- -- - - - //
                //   In the futur to be updated based on the condition need.
                ...
                function Update_select_field_for_new_table_selected(newValue){
                    let mytable_selected = base.getTableByNameIfExists(newValue);
                        ...
                        for (let l = 0; l < mytable_selected.fields.length; l++) {
                            if ((mytable_selected.fields[l].type == FieldType.COUNT) || (mytable_selected.fields[l].type == FieldType.CURRENCY) || (mytable_selected.fields[l].type  == FieldType.DATE)||
                            (mytable_selected.fields[l].type  == FieldType.DATE_TIME)||(mytable_selected.fields[l].type  == FieldType.DURATION)||(mytable_selected.fields[l].type == FieldType.FORMULA)||
                            (mytable_selected.fields[l].type  == FieldType.NUMBER)||(mytable_selected.fields[l].type == FieldType.PERCENT)||(mytable_selected.fields[l].type  == FieldType.RATING)){
                                 setmy_field_list(mytable_selected.fields[l].id);
                   
                            }
                        }
                        setvaluemy_field_targeted_list(mytable_selected.fields[0].id);
                    }
                }
                //----------------- END function to update the value select.
        
                // --------------- Const Select table where the table where the field for the condition is :
                //
                const Select_table = () => {
                    return (
                    <Select
                        ...
                        onChange={newValue => (setmy_table_list_value(newValue), Update_select_field_for_new_table_selected(newValue))}
                    />
                    );
                };
                //
                ...
                async function CreateVlookup_Indicator(){
                        ...
                        if (session.currentUser) {
                            ...
                            if(table_Lookup){
                                    ...
                                    // get ID of the table
                                    let my_table_list_value_id = "";
                                    for (let i = 0; i < my_table_list.length; i++) {
                                        if (my_table_list[i].value == my_table_list_value){
                                            my_table_list_value_id = my_table_list[i].id; 
                                        }
                                    }
            

                                const recordFields = {
                                    [myConstClass.FIELD_NAME_KPI] : name_input,
                                    [myConstClass.FIELD_NAMETABLE] : my_table_list_value_id,
                                    [myConstClass.FIELD_NAMEFIELD] : valuemy_field_list,
                                    [myConstClass.FIELD_NAMEFIELD_TARGETED] : valuemy_field_targeted_list,
                                    [myConstClass.FIELD_SELECTCONDITION] : {name: value_conditionvalue },
                                    [myConstClass.KPI_USER_AUTORIZE] : [user_id],
                                    }
                ...
                return (
                        <React.Fragment>
                            <Button onClick={() => setIsDialogOpen(true)} className ="Select_css1">Create VLookup indicator</Button>
                            {isDialogOpen && (
                                <Dialog onClose={() => setIsDialogOpen(false)} width="600px" position="fixed" bottom="-100" right="0" className ="Select_css2" >
                                <Dialog.CloseButton />
                                ...
                                <p> </p>
                                <div >
                                    <Button onClick={() => setIsDialogOpen(false)} text-align= "center"  margin="10px"  >Cancel</Button>
                                    <Button onClick={() => CreateVlookup_Indicator()}  text-align= "center"  margin=" 10px"  >Create KPI</Button>
                                </div>
                                </Dialog>
                            )}
                        </React.Fragment>
